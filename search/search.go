...
func (e *Engine) alphaBeta(depthLeft int8, searchHeight int8, alpha int16, beta int16) int16 {
	...
	if !isPvNode && ttHit && nDepth >= depthLeft && !firstLayerOfSingularity {
		if nEval >= beta && nType == LowerBound {
			e.CacheHit()
			e.searchHistory.AddHistory(nHashMove, currentMove, depthLeft, searchHeight, nil)
			return nEval
		}
			...
			if bestscore > alpha {
				if bestscore >= beta {
					if (e.isMainThread && !e.TimeManager().AbruptStop) || (!e.isMainThread && !e.parent.Stop) {
						...
						e.searchHistory.AddHistory(hashmove, currentMove, depthLeft, searchHeight, quietMoves)
					}
					return bestscore
				}
	...
	for true {
		...
		if oldEnPassant, oldTag, hc, ok := position.MakeMove(move); ok {
			...
			if e.doPruning && isQuiet && depthLeft > 2 && legalMoves > lmrThreashold {
				...
				LMR -= int8(e.searchHistory.History(currentMove, move) / 8192)
			...
			if score > bestscore {
				if score >= beta {
					if (e.isMainThread && !e.TimeManager().AbruptStop) || (!e.isMainThread && !e.parent.Stop) {
						...
						quietMoves := e.triedQuietMoves[searchHeight][:legalQuietMove]
						e.searchHistory.AddHistory(move, currentMove, depthLeft, searchHeight, quietMoves)
					}
					return score
				}
				// Potential PV move, lets copy it to the current pv-line
